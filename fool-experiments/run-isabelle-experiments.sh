...
for f in `cat isabelle-problems.txt`
do
  echo -n $f
  echo -en '\t'
  ...
  do
    ...
    VAMPIRE_OUTPUT=`${VAMPIRE} --mode ${VAMPIRE_MODE} -m 20000 -t ${TIME_LIMIT}s --fool_paramodulation on "smt_nada/$f.p"`
  ...
  do
    VAMPIRE_TIME="$(${DATE} +%s%6N)"
    VAMPIRE_OUTPUT=`${VAMPIRE} --mode ${VAMPIRE_MODE} -m 20000 -t ${TIME_LIMIT}s --fool_paramodulation off --fool_ordering off "smt_nada/$f.p"`
    VAMPIRE_TIME="$(($(${DATE} +%s%6N)-VAMPIRE_TIME))"
    echo -n "${VAMPIRE_MODE}"
    echo -en '\t'
    echo -n "${VAMPIRE_OUTPUT}" | sed -n 's/.*SZS status \([^ ]*\).*/\1/p' | tr -d '\n'
  ...
  CVC4_OUTPUT=`${TIMEOUT} 61s ${CVC4} --lang smt --output-lang tptp --tlimit=${TIME_LIMIT}000 smt_nada/${f}`
  ...
  Z3_TIME="$(${DATE} +%s%6N)"
  Z3_OUTPUT=`${Z3} -smt2 -T:${TIME_LIMIT} smt_nada/${f} 2>&-`
